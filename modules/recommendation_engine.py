...
def generate_recommendation(
    ...
    if ndvi < 0.2:
        ...
        recommendation["fertilizer"] = (
            "Balanced NPK fertilizer suggested"
        )
        ...
        recommendation["fertilizer"] = (
            "No immediate fertilizer needed"
        )
    ...
    if temp > 35:
        ...
        recommendation["irrigation"] += (
            " (Increase frequency due to heat stress)"
        )
    ...
    if crop == "Rice" and ndvi < 0.3:
        ...
        recommendation["fertilizer"] += (
            " | Paddy requires urgent nutrient correction"
        )

    if crop == "Wheat" and temp > 30:

        recommendation["risk"] += (
            " | Heat stress during grain formation"
        )
...
def render_recommendation_module():
    ...
    weather_data = st.session_state.get(
        "weather_data",
        None
    )

    ndvi_value = st.session_state.get(
        "ndvi_value",
        None
    )
    ...
    st.info(
        f"Irrigation: {result['irrigation']}"
    )

    st.info(
        f"Fertilizer: {result['fertilizer']}"
    )

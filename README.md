...
Identifies the geographic coordinates of the selected field.
Determines the corresponding administrative location.
Defines a spatial analysis region around the selected coordinate.
Retrieves satellite imagery for the analysis period.
Calculates vegetation condition using NDVI.
Retrieves location-specific weather information.
Combines environmental indicators into an interpretable field assessment.
Produces agricultural decision-support recommendations.
Generates a structured PDF report containing the analysis and field location map.
🎯 Objectives
...
Point-in-polygon administrative identification
Spatial buffering
Analysis region generation
